<script lang="ts">
	...
	interface Props {
		open?: boolean;
		page: PageDefinition;
		customizations: Record<string, FieldCustomization>;
		onclose?: () => void;
		onsave?: (customizations: Record<string, FieldCustomization>) => void;
	}

	let { open = false, page, customizations, onclose, onsave }: Props = $props();

	// Local copy of customizations for editing
	let localCustomizations = $state<Record<string, FieldCustomization>>({ ...customizations });
	...
	const drag = createDragAndDrop<FieldCustomization>();

	// Get all fields from all sections with applied order
	const allFields = $derived(() => {
		const fields: Array<DragItem<{ section: string; index: number }>> = [];
		let globalIndex = 0;
	...
	function toggleField(fieldSource: string, currentVisible: boolean) {
		localCustomizations = {
			...localCustomizations,
			[fieldSource]: {
	...
	// Get effective visibility for a field
	function getEffectiveVisibility(field: Field): boolean {
		// If user has customized this field, use that
	...
	// Get effective section for a field
	...
	function handleDropField(e: DragEvent, targetIndex: number) {
		localCustomizations = drag.handleDrop(e, targetIndex, allFields(), localCustomizations);
	}
</script>
...
<Modal {open} onclose={onclose} size="expanded">
	<div class="customize-modal">
		<div class="customize-header">
			<h2 class="customize-title">Customize Page: {page.page.caption}</h2>
			<p class="customize-subtitle">Show/hide fields, change sections, and drag to reorder</p>
		</div>

		<div class="customize-body">
			<div class="fields-list">
				{#each allFields() as item, idx}
					{@const isVisible = getEffectiveVisibility(item.field)}
					{@const currentSection = getEffectiveSection(item.field, item.data.section)}
					...
					>
						...
						<label class="field-checkbox" class:pointer-events-none={isDragging}>
							<input
								...
								onchange={(e) => {
									e.stopPropagation();
									toggleField(item.field.source, isVisible);
								}}
