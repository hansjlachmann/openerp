...
func (t *UserRoleBase) SetPage(limit, offset int) {
	if limit < 0 {
		limit = 0
	}
	if offset < 0 {
...
func (t *UserRoleBase) getLimitClause() string {
	if t.limit <= 0 {
		return ""
	}
	if t.offset > 0 {
...
func (t *UserRoleBase) FindSet() bool {
	...
	// Build SELECT with all fields
	query := fmt.Sprintf(`SELECT code, description FROM "%s" WHERE %s ORDER BY %s%s`, tableName, where, orderBy, t.getLimitClause())
