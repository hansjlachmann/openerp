...
func (t *JobQueueEntryBase) GetDBType() database.DBType {
	return t.dbType
}
...
func (t *JobQueueEntryBase) SetPage(limit, offset int) {
	if limit < 0 {
		limit = 0
	}
	if offset < 0 {
...
func (t *JobQueueEntryBase) getLimitClause() string {
	if t.limit <= 0 {
		return ""
	}
	if t.offset > 0 {
...
func (t *JobQueueEntryBase) FindSet() bool {
	...
	// Build SELECT with all fields
	query := fmt.Sprintf(`SELECT entry_no, job_queue_no, status, user_id, description, error_message, start_date_time, end_date_time FROM "%s" WHERE %s ORDER BY %s%s`, tableName, where, orderBy, t.getLimitClause())
