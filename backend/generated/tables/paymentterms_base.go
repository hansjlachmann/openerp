...
func (t *PaymentTermsBase) IsSetupTable() bool {
	return false
}
...
func (t *PaymentTermsBase) SetPage(limit, offset int) {
	if limit < 0 {
		limit = 0
	}
	if offset < 0 {
...
func (t *PaymentTermsBase) getLimitClause() string {
	if t.limit <= 0 {
		return ""
	}
	if t.offset > 0 {
...
func (t *PaymentTermsBase) FindSet() bool {
	...
	// Build SELECT with all fields
	query := fmt.Sprintf(`SELECT code, description, active FROM "%s" WHERE %s ORDER BY %s%s`, tableName, where, orderBy, t.getLimitClause())
