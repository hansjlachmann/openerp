...
func (t *UserMemberBase) Modify(runTrigger bool) bool {
	...
	sqlStr := fmt.Sprintf(`UPDATE "%s" SET %s WHERE user_id = ?role_id = ?company = ?`,
		tableName,
...
func (t *UserMemberBase) Delete(runTrigger bool) bool {
	...
	// Build SQL with placeholders
	sqlStr := fmt.Sprintf(`DELETE FROM "%s" WHERE user_id = ?role_id = ?company = ?`, tableName)
...
func (t *UserMemberBase) FindFirst() bool {
	...
	query := fmt.Sprintf(`SELECT user_id, role_id, company FROM "%s" WHERE %s ORDER BY user_idrole_idcompany ASC LIMIT 1`, tableName, where)
...
func (t *UserMemberBase) FindLast() bool {
	...
	// Build SELECT with all fields
	query := fmt.Sprintf(`SELECT user_id, role_id, company FROM "%s" WHERE %s ORDER BY user_idrole_idcompany DESC LIMIT 1`, tableName, where)
