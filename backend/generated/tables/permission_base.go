...
func (t *PermissionBase) SetPage(limit, offset int) {
	if limit < 0 {
		limit = 0
	}
	if offset < 0 {
...
func (t *PermissionBase) getLimitClause() string {
	if t.limit <= 0 {
		return ""
	}
	if t.offset > 0 {
...
func (t *PermissionBase) FindSet() bool {
	...
	// Build SELECT with all fields
	query := fmt.Sprintf(`SELECT role_id, table_name, can_read, can_insert, can_modify, can_delete FROM "%s" WHERE %s ORDER BY %s%s`, tableName, where, orderBy, t.getLimitClause())
